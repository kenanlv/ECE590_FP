...
namespace elma {
    ...
    Manager& Manager::watch(string event_name, std::function<void(Event&)> handler, int priority) {
        ...
        map_pri[event_name].push_back(priority);
        // pri.push_back(priority);
        int min;
        for (int i = 0; i < map_pri[event_name].size(); i++) {
            min = map_pri[event_name][i];
            for (int j = i + 1; j < map_pri[event_name].size(); j++) {
                if (min > map_pri[event_name][j]) {
                    min = map_pri[event_name][j];
                    swap(&map_pri[event_name][i], &map_pri[event_name][j]);
                    swap(&event_handlers[event_name][i], &event_handlers[event_name][j]);
                }
            }

        // std::cout<<std::to_string(pri[i])<<" pri value\n";
        }
        // for (int k = 0; k < map_pri[event_name].size(); k++){
        //     std::cout<<map_pri[event_name][k]<<" pri value\n";
        // }
        // std::cout<<"\n";
        return *this;
    }
